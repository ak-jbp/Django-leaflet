L.Control.StyledLayerControl = L.Control.Layers.extend({
  ...
  L.marker([39.61, -105.02]).addTo(layer1);
  L.marker([39.74, -104.99]).addTo(layer2);
  L.marker([39.73, -104.8]).addTo(layer3);
  L.marker([39.77, -105.23]).addTo(layer4);
  L.marker([39.87, -105.23]).addTo(layer5);
  L.marker([39.64, -105.34]).addTo(layer6);
  
  var cdbAttr = 'Map data ';
  var cdbUrl = 'http://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}.png';
  var light = L.tileLayer(cdbUrl, {
    attribution: cdbAttr
  });
  ...
  var options = {
    ...
    group_maxHeight: "60px",
    exclusive: false
  };
  
  //Create the Styled Layer Control
  var control = L.Control.styledLayerControl(baseLayer, overlays, options);
  map.addControl(control);
  
  $('#leaflet-control-accordion-layers-0').appendTo('#1'); 
  $('#leaflet-control-accordion-layers-1').appendTo('#2'); 
  //$('#leaflet-control-accordion-layers-2').appendTo('#3'); 
